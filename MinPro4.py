...
class ManajemenKeyboard():
    ...
    def jump_search(self, idKeyboard, descending=False):
        daftarId = []
        current = self.head
        while current:
            daftarId.append(current.idKeyboard)
            current = current.next

        if descending:
            urutDaftarId = self.quickSort(daftarId, descending=True)
        else:
            urutDaftarId = self.quickSort(daftarId)

        n = len(urutDaftarId)
        step = int(n ** 0.5)
        prev = 0
        while prev < n and urutDaftarId[min(step, n) - 1] < idKeyboard:
            prev = step
            step += int(n ** 0.5)
            if prev >= n:
                print("Keyboard Tidak Ada.")
                return
        while urutDaftarId[prev] < idKeyboard:
            prev += 1
            if prev == min(step, n):
                print("Keyboard Tidak Ada.")
                return
        if urutDaftarId[prev] == idKeyboard:
            current = self.head
            while current:
                if current.idKeyboard == idKeyboard:
                    print("Keyboard Ditemukan!")
...
def cariKeyboard():
    idKeyboard = str(input("Masukkan ID Keyboard: "))
    clear()
    manajemenKeyboardobj.jump_search(idKeyboard)
    input("Tekan enter untuk melanjutkan...")
